...
[Github](https://github.com/) - especially the issue and discussions - is a brillian ressources, but may be a little to technical for starters.
You can find tutorials online, or look in the package documentation on CRAN or the packages Website.
This all requires you to switch from `R` to your browser, viewer etc.
Sometimes, you just need a quick fix, the `?` command is here to help.
It calls the `R` in-built help and documentation and is a goto resources for simply *looking stuff up* because let's face it: who can remember everything?
...
`R` offers the `citation()` command to create `bibtex` entries that can be used by practicall all reference management software.
